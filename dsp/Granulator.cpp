...
Granulator::RTGrainVoice::RTGrainVoice(Granulator* voiceOwner, int voiceIndex) : _voiceOwner(voiceOwner), _voiceIndex(voiceIndex) {}
Granulator::RTGrainVoice:: ~RTGrainVoice()
{
    for (int i = 0; i < 2; i++) {
...
Granulator* Granulator::getTopLevelPatcher() {
    return this;
}
...
template <typename T> void Granulator::listquicksort(T& arr, T& sortindices, Int l, Int h, bool ascending) {
    if (l < h) {
...
template <typename T> Int Granulator::listpartition(T& arr, T& sortindices, Int l, Int h, bool ascending) {
...
template <typename T> void Granulator::listswapelements(T& arr, Int a, Int b) {
...
array<ListNum, 2> Granulator::listcompare(const list& input1, const list& input2) {
    array<ListNum, 2> tmp = {};
    tmp[0] = 0;
    tmp[1] = {};
...
number Granulator::beatstohz(number beattime) {
    return this->safediv(this->tempo(), beattime * 60);
}

ParameterValue Granulator::tonormalized(ParameterIndex index, ParameterValue value) {
    return this->convertToNormalizedParameterValue(index, value);
}

number Granulator::hztobeats(number hz) {
    return this->safediv(this->tempo() * 8, hz * 480.);
}

number Granulator::beattimeatsample(number offset) {
    return this->getTopLevelPatcher()->globaltransport_getBeatTimeAtSample(offset);
}

number Granulator::transportatsample(SampleIndex sampleOffset) {
    return this->getTopLevelPatcher()->globaltransport_getStateAtSample(sampleOffset);
}

ParameterValue Granulator::fromnormalized(ParameterIndex index, ParameterValue normalizedValue) {
    return this->convertFromNormalizedParameterValue(index, normalizedValue);
}

number Granulator::tickstohz(number ticks) {
...
void Granulator::processMidiEvent(MillisecondTime time, int port, ConstByteArray data, Index length) {
...
Index Granulator::getNumMidiOutputPorts() const {
    return 0;
}
...
void Granulator::prepareToProcess(number sampleRate, Index maxBlockSize, bool force) {
    if (this->maxvs < maxBlockSize || !this->didAllocateSignals) {
        Index i;
...
void Granulator::setProbingTarget(MessageTag id) {
    switch (id) {
    default:
    {
...
void Granulator::setProbingIndex(ProbingIndex) {}

Index Granulator::getProbingChannels(MessageTag outletId) const {
    RNBO_UNUSED(outletId);
    return 0;
}

DataRef* Granulator::getDataRef(DataRefIndex index) {
    switch (index) {
    case 0:
    {
...
DataRefIndex Granulator::getNumDataRefs() const {
    return 3;
}

void Granulator::fillDataRef(DataRefIndex, DataRef&) {}

void Granulator::zeroDataRef(DataRef& ref) {
    ref->setZero();
}

void Granulator::processDataViewUpdate(DataRefIndex index, MillisecondTime time) {
    this->updateTime(time);
...
Index Granulator::getPatcherSerial() const {
    return 0;
}

void Granulator::getState(PatcherStateInterface&) {}

void Granulator::setState() {
    for (int i = 0; i < 24; i++) {
...
void Granulator::getPreset(PatcherStateInterface& preset) {
...
void Granulator::setPreset(MillisecondTime time, PatcherStateInterface& preset) {
    this->updateTime(time);
...
void Granulator::processTempoEvent(MillisecondTime time, Tempo tempo) {
    this->updateTime(time);
...
void Granulator::processParameterEvent(ParameterIndex index, ParameterValue value, MillisecondTime time) {
    this->setParameterValue(index, value, time);
}

void Granulator::processParameterBangEvent(ParameterIndex index, MillisecondTime time) {
    this->setParameterValue(index, this->getParameterValue(index), time);
}

void Granulator::processNormalizedParameterEvent(ParameterIndex index, ParameterValue value, MillisecondTime time) {
    this->setParameterValueNormalized(index, value, time);
}

ParameterValue Granulator::getParameterValue(ParameterIndex index) {
    return paramvalues[index];
}

ParameterIndex Granulator::getNumSignalInParameters() const {
    return 0;
}

ParameterIndex Granulator::getNumSignalOutParameters() const {
    return 0;
}

ParameterIndex Granulator::getNumParameters() const {
    return 21;
}

ConstCharPointer Granulator::getParameterName(ParameterIndex index) const {
    return getParameterId(index);
}

ConstCharPointer Granulator::getParameterId(ParameterIndex index) const {
    ParameterInfo pinfo;
    this->getParameterInfo(index, &pinfo);
...
void Granulator::getParameterInfo(ParameterIndex index, ParameterInfo* info) const {
    {
        switch (index) {
        case 0:
        {
...
ParameterIndex Granulator::getParameterOffset(BaseInterface* subpatcher) const { return 0; }

ParameterValue Granulator::applyStepsToNormalizedParameterValue(ParameterValue normalizedValue, int steps) const {
    if (steps == 1) {
...
ParameterValue Granulator::convertToNormalizedParameterValue(ParameterIndex index, ParameterValue value) const {
    ParameterInfo pinfo;
    this->getParameterInfo(index, &pinfo);
...
ParameterValue Granulator::convertFromNormalizedParameterValue(ParameterIndex index, ParameterValue value) const {
    ParameterInfo pinfo;
    this->getParameterInfo(index, &pinfo);
...
void Granulator::scheduleParamInit(ParameterIndex index, Index order) {
    this->paramInitIndices->push(index);
    this->paramInitOrder->push(order);
}

void Granulator::processParamInitEvents() {
    this->listquicksort(
        this->paramInitOrder,
...
void Granulator::processClockEvent(MillisecondTime time, ClockId index, bool hasValue, ParameterValue value) {}

void Granulator::processOutletAtCurrentTime(EngineLink*, OutletIndex, ParameterValue) {}

void Granulator::processOutletEvent(
    EngineLink* sender,
    OutletIndex index,
...
void Granulator::processNumMessage(MessageTag tag, MessageTag objectId, MillisecondTime time, number payload) {}

void Granulator::processListMessage(
    MessageTag tag,
    MessageTag objectId,
    MillisecondTime time,
    const list& payload
...
void Granulator::processBangMessage(MessageTag tag, MessageTag objectId, MillisecondTime time) {}

MessageTagInfo Granulator::resolveTag(MessageTag tag) const {
    switch (tag) {
    case TAG("valout"):
    {
        return "valout";
...
MessageIndex Granulator::getNumMessages() const { return 0; }

const MessageInfo& Granulator::getMessageInfo(MessageIndex index) const { return NullMessageInfo; }

void Granulator::updateVoiceState(int voice, voiceState voicestate) {
...
void Granulator::limi_01_preamp_setter(number v) {
    this->limi_01_preamp = rnbo_pow(10., v * 0.05);
}

void Granulator::limi_01_postamp_setter(number v) {
    this->limi_01_postamp = rnbo_pow(10., v * 0.05);
}

void Granulator::limi_01_threshold_setter(number v) {
    this->limi_01_threshold = rnbo_pow(10., v * 0.05);
}

number Granulator::limi_01_dc1_next(number x, number gain) {
...
void Granulator::limi_01_dc1_reset() {
    this->limi_01_dc1_xm1 = 0;
    this->limi_01_dc1_ym1 = 0;
}

void Granulator::limi_01_dc1_dspsetup() {
    this->limi_01_dc1_reset();
}

number Granulator::limi_01_dc2_next(number x, number gain) {
...
void Granulator::limi_01_dc2_reset() {
    this->limi_01_dc2_xm1 = 0;
    this->limi_01_dc2_ym1 = 0;
}

void Granulator::limi_01_dc2_dspsetup() {
    this->limi_01_dc2_reset();
}

number Granulator::limi_01_dc_next(Index i, number x, number gain) {
    switch ((int)i) {
    case 0:
    {
...
void Granulator::limi_01_dc_dspsetup(Index i) {
    switch ((int)i) {
    case 0:
    {
...
void Granulator::limi_01_dspsetup(bool force) {
...
void Granulator::dcblock_tilde_01_dspsetup(bool force) {
...
number Granulator::globaltransport_getTempoAtSample(SampleIndex sampleOffset) {
    return (sampleOffset >= 0 && sampleOffset < this->vs ? this->globaltransport_tempo[(Index)sampleOffset] : this->globaltransport_lastTempo);
}

number Granulator::globaltransport_getStateAtSample(SampleIndex sampleOffset) {
    return (sampleOffset >= 0 && sampleOffset < this->vs ? this->globaltransport_state[(Index)sampleOffset] : this->globaltransport_lastState);
}

number Granulator::globaltransport_getState(MillisecondTime time) {
    return this->globaltransport_getStateAtSample(this->globaltransport_getSampleOffset(time));
}

number Granulator::globaltransport_getBeatTime(MillisecondTime time) {
    number i = 2;
...
bool Granulator::globaltransport_setTempo(MillisecondTime time, number tempo, bool notify) {
    if ((bool)(notify)) {
...
number Granulator::globaltransport_getTempo(MillisecondTime time) {
    return this->globaltransport_getTempoAtSample(this->globaltransport_getSampleOffset(time));
}

bool Granulator::globaltransport_setState(MillisecondTime time, number state, bool notify) {
    if ((bool)(notify)) {
...
bool Granulator::globaltransport_setBeatTime(MillisecondTime time, number beattime, bool notify) {
    if ((bool)(notify)) {
...
number Granulator::globaltransport_getBeatTimeAtSample(SampleIndex sampleOffset) {
    auto msOffset = this->sampstoms(sampleOffset);
    return this->globaltransport_getBeatTime(this->getEngine()->getCurrentTime() + msOffset);
}

array<number, 2> Granulator::globaltransport_getTimeSignature(MillisecondTime time) {
    number i = 3;
...
array<number, 2> Granulator::globaltransport_getTimeSignatureAtSample(SampleIndex sampleOffset) {
    auto msOffset = this->sampstoms(sampleOffset);
    return this->globaltransport_getTimeSignature(this->getEngine()->getCurrentTime() + msOffset);
}

bool Granulator::globaltransport_setTimeSignature(MillisecondTime time, number numerator, number denominator, bool notify) {
    if ((bool)(notify)) {
...
void Granulator::globaltransport_advance() {
...
void Granulator::globaltransport_dspsetup(bool force) {
...
void Granulator::assign_defaults()
{
    limi_01_bypass = 0;
    limi_01_dcblock = 0;
